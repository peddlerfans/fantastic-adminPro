...
const appRadius = computed<number[]>({
  get() {
    return [userStore.preferences.app?.radius ?? 0.5]
  },
  set(value) {
    userStore.preferences.app!.radius = value[0]
  },
})
...
<template>
  <FaDrawer v-model="isShow" title="偏好设置" :side="settingsStore.settings.app.direction === 'ltr' ? 'right' : 'left'" content-class="sm:min-w-md">
    <div
      v-if="
    ...
    <div v-if="settingsStore.mode === 'pc' && (userStore.preferences.layout?.hasOwnProperty('widthMode') || userStore.preferences.layout?.hasOwnProperty('widthModeScope'))">
      <div class="divider">
        页宽模式
      </div>
      <div v-if="userStore.preferences.layout?.hasOwnProperty('widthMode')" class="app-width-mode">
      ...
      <div v-if="userStore.preferences.layout?.hasOwnProperty('widthModeScope')" class="setting-item">
        <div class="label">
          作用范围
        </div>
        <div class="flex-center-start gap-1">
          <FaButton
            ...
            ]" :key="index" :variant="userStore.preferences.layout.widthModeScope === item.value ? 'default' : 'outline'" size="sm" class="w-12" @click="userStore.preferences.layout?.hasOwnProperty('widthModeScope') && (userStore.preferences.layout.widthModeScope = (item.value as any))"
          >
            {{ item.label }}
          </FaButton>
        </div>
