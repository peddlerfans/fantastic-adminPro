...
async function fetchUserStatistics() {
  ...
  const res = await testApi.rewardStatistics({ startDate, endDate })
  total.value = res.data.totalCommissionAmount
  today.value = res.data.todayCommissionAmount || 0
  chartData.value = res.data.dailyCommissionAmounts || []
  option.value = {
    type: 'line',
    data: [{ id: "reward", values: chartData.value }],
    xField: 'date',
    yField: 'count',
    height: 300,
  }
...
<template>
  <FaCard title="奖励报表" class="mb-4">
    <div class="mb-6 flex gap-8">
      ...
      <div class="flex-1 rounded-lg from-red-100 to-red-50 bg-gradient-to-r p-6 shadow">
